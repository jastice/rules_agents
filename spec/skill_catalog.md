...
- `--json`
  - print the full aggregate manifest as JSON; filter flags are not applied in JSON
    mode
- `--registry=<id>`
...
- `@rules_agents_registry_index//:list_skills` is a generated `sh_binary` with a data
  dependency on `@rules_agents_registry_index//:aggregate_manifest.json`
- the aggregate manifest is present in runfiles at runtime
- the formatter reads only that aggregate manifest; for `--json` output it emits the manifest
  verbatim, for human-readable output it formats pre-baked text blocks generated by Starlark
...
- the generated shell script must not depend on any runtime beyond POSIX shell and standard
  utilities (`cat`, `awk`)
...
`list_skills --json` must emit this full aggregate schema. Filter flags (`--registry`,
`--skill`, `--agent`) are not applied in JSON mode. Consumers that need filtered JSON should
pipe through `jq` or equivalent.
...
`--json` prints the full aggregate manifest schema from section 10.2. Filter flags
(`--registry`, `--skill`, `--agent`) are not applied in JSON mode. This keeps the formatter
trivial and avoids a runtime JSON processing dependency; consumers that need filtered results
should pipe through `jq` or equivalent.
...
`--skill=<name>` matching rules (human-readable output only; filters are not applied in
`--json` mode):
...
### 15.1 Runtime dependency constraint

`@rules_agents_registry_index//:list_skills` must not require an external language runtime
beyond POSIX shell. The formatter is generated as a shell script by the repository rule with
all discovery data pre-baked, so consuming repositories do not need `rules_python` or any
other toolchain dependency to run the listing target.

### 15.2 Suggested new files
...
- the generated `list_skills` target requires only POSIX shell and standard utilities
- `--json` emits the full unfiltered aggregate manifest
- filter flags apply only to human-readable output
